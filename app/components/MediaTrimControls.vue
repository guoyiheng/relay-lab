...
<template>
  <section class="trim-controls w-full shrink-0 rounded-lg border border-white/15 bg-white/[0.06] p-4 text-white" aria-label="截取参考素材">
    <div class="flex flex-wrap items-center justify-between gap-2 text-sm">
      <h2 class="font-medium">截取参考素材</h2>
    ...
    <fieldset :disabled="processing" class="grid gap-4 sm:grid-cols-2">
      <div>
        <label class="flex items-center justify-between gap-2 text-xs">
          <span>开始时间（秒）</span>
          <input v-model="startDraft" type="number" min="0" :max="Math.max(0, end - gap)" step="0.1" :disabled="processing || !ready" class="trim-time" @change="commit('start')" />
        </label>
        <input :value="start" type="range" min="0" :max="duration || 1" step="0.001" :disabled="processing || !ready" aria-label="拖动选择开始时间" :aria-valuetext="formatMediaTime(start)" class="trim-range" @input="slide('start', $event)" />
        <button type="button" class="trim-link" :disabled="processing || !ready || currentTime > end - gap" @click="emit('start', currentTime)">将当前进度设为开始</button>
      </div>
      <div>
        <label class="flex items-center justify-between gap-2 text-xs">
          <span>结束时间（秒）</span>
          <input v-model="endDraft" type="number" :min="start + gap" :max="duration" step="0.1" :disabled="processing || !ready" class="trim-time" @change="commit('end')" />
        </label>
        <input :value="end" type="range" min="0" :max="duration || 1" step="0.001" :disabled="processing || !ready" aria-label="拖动选择结束时间" :aria-valuetext="formatMediaTime(end)" class="trim-range" @input="slide('end', $event)" />
        <button type="button" class="trim-link" :disabled="processing || !ready || currentTime < start + gap" @click="emit('end', currentTime)">将当前进度设为结束</button>
      </div>
    </fieldset>
    ...
    <div class="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-white/10 pt-3">
      ...
      <div class="flex flex-wrap gap-2">
        <button type="button" class="trim-button border border-white/25 text-white/85 hover:bg-white/10" :disabled="processing || !ready" @click="emit('reset')">重置</button>
        <button type="button" class="trim-button border border-white/25 text-white/85 hover:bg-white/10" :disabled="processing || !ready || !!error" @click="emit('play')">{{ playing ? '暂停播放' : '播放选区' }}</button>
        <button type="button" class="trim-button bg-primary-500 text-white hover:bg-primary-600" :disabled="processing || !ready || !!error || end - start < 0.1" @click="emit('apply')">{{ processing ? '正在生成…' : '截取并替换参考' }}</button>
      </div>
    </div>
